function bbs = edgeBoxes_F( I, boxes, featmap, model, varargin )
% Modified code of Edge Boxes object proposals as described in:
...
%  Jan Hosang, Rodrigo Benenson, Piotr Doll�r, and Bernt Schiele
...
% run detector over pre-defined boxes
f=o.name; if(~isempty(f) && exist(f,'file')), bbs=1; return; end
bbs=edgeBoxesImg(I,model,o, boxes, featmap);

end

% refinement based on Edgebox algorithm
function bbs = edgeBoxesImg( I, model, o, boxes, featmap )

if(all(ischar(I))), I=imread(I); end
model.opts.nms=0; [E,O]=edgesDetect_F(I, model, featmap);
