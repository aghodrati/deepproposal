...
void mexFunction( int nl, mxArray *pl[], int nr, const mxArray *pr[] )
{
  ...
  if(nr != 15) mexErrMsgTxt("Fifteen inputs required.");
  if(nl > 2) mexErrMsgTxt("At most two outputs expected.");
  if(mxGetClassID(pr[14])!=mxSINGLE_CLASS) mexErrMsgTxt("boxes must be single");
  ...
  float* boxes_array = (float*)mxGetPr( pr[14] );
  int nbox = (int) mxGetM(pr[14]);  //number of input boxes
  ...
  for(int i=0; i<n; i++) {
    ...
    bbs[ i + 2*n ] = (float) (boxes[i].w+boxes[i].c);
    bbs[ i + 3*n ] = (float) (boxes[i].h+boxes[i].r);
    bbs[ i + 4*n ] = boxes[i].s;
  }
}
